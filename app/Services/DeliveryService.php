...
use App\Mail\PhotoDeliveryMail;
    ...
    public function sendPhotoAccess(Order $order)
    {
        try {
            ...
            if ($waToken && $phone) {
                ...
                //kirim ke email juga
                // Mail::to($order->customer_email)->send(new PhotoDeliveryMail($order, $downloadUrl));
                // Log::info('Email Delivery Success Sent to: ' . $order->customer_email);
            ...
            Log::error('Failed to send photo access email or WhatsApp: ' . $e->getMessage());
        }
    }
}
